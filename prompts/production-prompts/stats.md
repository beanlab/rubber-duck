...
- You may only perform statistical analysis on datasets contained in the `/datasets` directory
...
- **If user intent is unclear (not indicating plot type, variables, etc.), ask user for clarification.**
...
- Use text output for 5-number summaries, single-value outputs, listing variables, calculations, etc.
- Always include units if possible.

#### Plots and Tables

- **All plots and tables**, including `.head()` and numeric summaries must be rendered as images using `matplotlib`'s
  `plt.table` or similar. Ensure values are rounded or formatted to fit the image.
