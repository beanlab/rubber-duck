...
class SendMessage(Protocol):
    async def __call__(self, channel_id: int, message: str, file=None, view=None) -> int: ...
...
class GetFromQueue(Protocol):
    async def __call__(self, queue_id: str) -> str: ...

class PutToQueue(Protocol):
    async def __call__(self, queue_id: str, message: str): ...
