...
class AssignmentFeedbackWorkflow:
    ...
    def _get_nested(self, d, keys):
        return reduce(operator.getitem, keys, d) # TODO learn reduce
    ...
    async def _get_report_contents(self, context):
        try:
            ...
            while True: # do in range 3
                response = await wait_for_message(context.timeout)
                if response is None:
                    raise CloseConversationException("Timed Out ")
            ...
            duck_logger.warning(f"Exception: {e}") # the code is broken
