...
class Registration:
    ...
    async def run(self, context: DuckContext):
        # Start the registration process
        net_id = await self.get_net_id(context)
        if not net_id:
            await self._send_message(context.thread_id, "Registration failed: No Net ID provided. Please start over.")
            raise Exception("No Net ID Provided")
        ...
        if not self.check_net_id(net_id):
            ...
            raise Exception("Net ID did not meet given requirements")


        # Get and verify the email
        if not await self.confirm_registration_via_email(context, net_id):
            await self._send_message(context.thread_id,
                                     'Unable to validate your email. Please talk to a TA or your instructor.')
            raise Exception("Did not confirm with code in allotted time")

        # Assign Discord roles
        await self.assign_roles(context)

        # Get and assign nickname as well as resolve issues with questionable names
        await self.nickname_flow(context)
