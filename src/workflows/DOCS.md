...
- `CodeDuckWorkflow` sends the configured opening message and initial project directly, builds a script-style `Student:`/`TA:` transcript for conversation review, then sends the conversation-review response directly to the thread.
...
- Uses one `AIClient` agent for code-duck conversation review and student-style debugging follow-up generation.
...
- Code-duck conversation-review configs do not need `output_format`; the workflow clears review output formatting, preserves configured tools such as `conclude_conversation`, and treats text responses as the next student-facing message.
- Code-duck has no separate user-facing code duck agent; only conversation review receives the full rubric and transcript.
